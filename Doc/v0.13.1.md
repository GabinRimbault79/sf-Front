...
- Fixed `Input` label and input association
- Fixed external link handling in `List`
- Decoupled `Button`, `Img`, and `Tag` from `react-router-dom` for basic usage
- Improved accessibility on key interactive components
- Removed risky default props and unsafe fallback behavior in core components
...
This release improves the packaging and documentation of the library.

### Packaging improvements

- React and ReactDOM are now handled as peer dependencies
- development-only dependencies were removed from runtime dependencies
- package generation was validated locally before publication
- a public export issue related to `WrapperExample` was fixed

### Documentation

- the README was cleaned up and clarified
- contribution and release notes were moved into `CONTRIBUTING.md`
- peer dependency requirements are now documented more clearly

### Known follow-up

- cross-platform build validation is still planned for a later step
